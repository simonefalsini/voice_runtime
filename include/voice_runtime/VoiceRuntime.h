...
public:
  VoiceRuntime(RuntimeConfig config,
      ...
      : cfg_(std::move(config)), audioPool_(cfg_.audioPoolBuffers)
        // --- code audio ---
        ,
        micRawQueue_(cfg_.micRawQueueCapacity, cfg_.audioOverflowPolicy,
                     "micRaw"),
        micQueue_(cfg_.micQueueCapacity, cfg_.audioOverflowPolicy,
                  "micAdapted"),
        vadGatedQueue_(cfg_.vadGatedQueueCapacity, cfg_.audioOverflowPolicy,
                       "vadOut"),
        ttsRefQueue_(cfg_.ttsReferenceQueueCapacity,
                     QueueOverflowPolicy::BlockProducer, "ttsRef"),
        speakerQueue_(cfg_.speakerQueueCapacity,
                      QueueOverflowPolicy::BlockProducer, "speaker"),
        cleanQueue_(cfg_.cleanAudioQueueCapacity, cfg_.audioOverflowPolicy,
                    "cleanAudio"),
        sttAdaptedQueue_(cfg_.sttAdaptedQueueCapacity, cfg_.audioOverflowPolicy,
                         "sttAdapted")
        // --- code testo ed eventi ---
        ,
        sttTextQueue_(cfg_.sttTextQueueCapacity, cfg_.textOverflowPolicy,
                      "sttText"),
        classifierOutQueue_(cfg_.classifierOutQueueCapacity,
                            cfg_.textOverflowPolicy, "classOut"),
        llmOutputQueue_(cfg_.llmOutputQueueCapacity, cfg_.textOverflowPolicy,
                        "llmOut"),
        ttsInputQueue_(cfg_.ttsInputQueueCapacity, cfg_.textOverflowPolicy,
                       "ttsIn"),
        vadEventQueue_(cfg_.vadEventQueueCapacity,
                       QueueOverflowPolicy::DropOldest, "vadEvents"),
        bargeInQueue_(cfg_.bargeInEventQueueCapacity,
                      QueueOverflowPolicy::DropOldest, "bargeIn")
        // --- buffer LLM ---
        ,
        llmDiskBuffer_(cfg_.llmDiskSpoolPath, cfg_.llmMemoryWindowBytes)
        // --- buffer overlap ---
        ,
        overlapBuffer_(cfg_.overlapCommentBufferBytes,
                       cfg_.overlapBufferUnbounded)
        // --- nodi ---
        ,
        microphone_(std::move(microphone)), aec_(std::move(aec)),
        stt_(std::move(stt)), classifier_(std::move(classifier)),
        llm_(std::move(llm)), interpreter_(std::move(interpreter)),
        tts_(std::move(tts)), output_(std::move(output)) {
    buildAdapters();
  }
  ...
  bool initialize() {
    ...
    const bool needMicAdapter =
        cfg_.enableMicAdapter && (cfg_.micRawFormat != cfg_.pipelineFormat);
    ...
    const bool useIntegratedVad =
        cfg_.enableIntegratedDspVad &&
        (dynamic_cast<IVadNode *>(aec_.get()) != nullptr);
    const bool useExternalVad = cfg_.enableVad && vad_;

    if (useExternalVad) {
      vad_->setInputQueue(&micQueue_);
      vad_->setOutputQueue(&cleanQueue_);
      vad_->setEventQueue(&vadEventQueue_);
      vad_->setSpeechThreshold(cfg_.vadSpeechThreshold);
      vad_->setTtsStateSignal(&ttsState_);
      vad_->setACRStateSignal(&ACRState_);
    }
    ...
    aec_->setACRStateSignal(&ACRState_);
    ...
    const bool needSttAdapter =
        cfg_.enableSttAdapter && (cfg_.pipelineFormat != cfg_.sttInputFormat);
    ...
    const bool ok = microphone_->initialize() && aec_->initialize() &&
                    stt_->initialize() && classifier_->initialize() &&
                    llm_->initialize() && interpreter_->initialize() &&
                    tts_->initialize() && output_->initialize() &&
  ...
  void start() {
    ...
    if (!state_.compare_exchange_strong(
            expected, static_cast<int>(RuntimeState::Running)))
      return;

    if (cfg_.printMetrics)
      setupMetrics();
  ...
  void stop() {
    ...
    if (!state_.compare_exchange_strong(
            expected, static_cast<int>(RuntimeState::Stopping)))
      return;
    ...
    // 4. Svuota le code (rilascia shared_ptr prima che i pool vengano
    // distrutti)
    clearAllQueues();
  ...
  void setVadNode(std::unique_ptr<IVadNode> vad) { vad_ = std::move(vad); }
  ...
  TtsStateSignal &acrState() { return ACRState_; }
...
private:
  void buildAdapters() {
    ...
    if (cfg_.enableMicAdapter && cfg_.micRawFormat != cfg_.pipelineFormat) {
      micAdapter_ = std::make_unique<AudioFormatAdapterNode>(
          "MicAdapter", cfg_.micRawFormat, cfg_.pipelineFormat,
          adapterPoolSize);
    }

    if (cfg_.enableSttAdapter && cfg_.pipelineFormat != cfg_.sttInputFormat) {
      sttAdapter_ = std::make_unique<AudioFormatAdapterNode>(
          "SttAdapter", cfg_.pipelineFormat, cfg_.sttInputFormat,
          adapterPoolSize);
    }
  }
  ...
  void setupMetrics() {
    ...
    metrics_->setExtraLineCallback([this]() -> std::string {
      ...
      const bool ttsActive = ttsState_.isActive();
        const bool ACRStateActive = ACRState_.isActive();
      std::snprintf(
          buf, sizeof(buf),
          "pool=%zu/%zu free | llmMem=%zu B | tts=%s | ttsInt=%s | ACRInt=%s | llmInt=%s",
          poolFree, poolCap, diskBytes, ttsActive ? "ACTIVE" : "idle",ACRStateActive ? "ACTIVE" : "idle",
