...
class SimulatedMicrophoneNode final : public ActiveNodeBase,
                                      public IMicrophoneNode {
public:
  SimulatedMicrophoneNode(AudioFormat format, int framesPerSecond,
                          std::size_t poolSize)
...
class SimulatedVadNode final : public ActiveNodeBase, public IVadNode {
public:
...
private:
  void toggleSpeech(uint64_t nowNs) {
    ...
    VadEvent ev;
    ev.type =
        wasSpeaking ? VadEvent::Type::SpeechEnd : VadEvent::Type::SpeechStart;
...
class SimulatedClassifierBargeInNode final : public ActiveNodeBase,
...
private:
  ...
  void doBargeIn(BargeInEvent::Source src, const std::string &reason) {
    ...
    const char *srcLabel =
        (src == BargeInEvent::Source::Keyboard) ? "Keyboard" : "Context";
...
class SimulatedLlmNode final : public ActiveNodeBase,
                               public ILanguageModelNode {
public:
  explicit SimulatedLlmNode(int processMsPerChunk)
      : processMs_(processMsPerChunk) {}
...
protected:
  void runLoop() override {
    while (running()) {
      TextChunk input;
      if (!in_ || !in_->pop(input))
        break;

      if (disk_)
        disk_->append(input.text + "\n");
...
class SimulatedInterpreterNode final : public ActiveNodeBase,
...
public:
  ...
  void setInterruptSignal(InterruptSignal *s) override { intSig_ = s; }
  void setTtsStateSignal(TtsStateSignal *signal) override {
    ttsState_ = signal;
  }
...
class SimulatedAudioOutputNode final : public ActiveNodeBase,
