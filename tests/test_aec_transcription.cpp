...
//     TTS active   → SileroVAD passes through, WebRtcDSP does AEC + internal
//     VAD
//
// Flow:
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 5) {
    ...
    if (sttModelPath.find("1.7B") != std::string::npos) {
      ...
      std::string dir =
          (pos == std::string::npos) ? "" : sttModelPath.substr(0, pos + 1);
  ...
  std::printf(
      ...
      kBold, kCyan, kReset);
  std::printf(
      "%s%s║  🔊  Echo Cancellation Transcription Test                 ║%s\n",
      kBold, kCyan, kReset);
  std::printf(
      "%s%s╚══════════════════════════════════════════════════════════╝%s\n\n",
      kBold, kCyan, kReset);
  ...
  AudioFrameQueue cleanQueue(512, QueueOverflowPolicy::DropOldest,
                             "cleanQueue");
  ...
  AudioFrameQueue aecRefQueue(32, QueueOverflowPolicy::BlockProducer,
                              "aecRefQueue");
  // TTS → speaker output (24kHz)
  AudioFrameQueue speakerQueue(256, QueueOverflowPolicy::BlockProducer,
                               "speakerQueue");
  // TTS text input
  TextQueue ttsInputQueue(128, QueueOverflowPolicy::DropOldest,
                          "ttsInputQueue");
  // STT text output
  ...
  // no vad NODE the DSP is active all time
  ...
  std::printf(
      "%s[Config]%s WebRTC VAD Mode: %d | Hangover: %d ms | Gating: %s\n",
  ...
  while (g_running.load()) {
    ...
    while (textQueue.tryPop(chunk)) {
      ...
      const char *modeLabel = ttsState.isActive() ? "🔇 AEC+STT" : "🎤 VAD+STT";

      std::printf("\n  %s%s[%d] %s:%s %s%s%s\n\n", kBold, kCyan,
                  transcriptionCount, modeLabel, kReset, kBold,
                  chunk.text.c_str(), kReset);
      std::fflush(stdout);
      ...
      if (currentUserLine < static_cast<int>(userLines.size())) {
        ...
        if (currentUserLine < static_cast<int>(userLines.size())) {
          std::printf(
              "  %s▶ Ora leggi la riga %d:%s %s%s%s\n\n", kGreen,
              currentUserLine + 1, kReset, kYellow,
  ...
  // Stop queues first to unblock threads (except textQueue so STT can push
  // final flush)
  micQueue.stop();
  cleanQueue.stop();
  aecRefQueue.stop();
  ...
  std::printf(
      "  micQueue:      prod=%-6llu  cons=%-6llu  drop=%-4llu  hwm=%zu\n",
  ...
  std::printf(
      "  cleanQueue:    prod=%-6llu  cons=%-6llu  drop=%-4llu  hwm=%zu\n",
  ...
  std::printf(
      "  aecRefQueue:   prod=%-6llu  cons=%-6llu  drop=%-4llu  hwm=%zu\n",
  ...
  std::printf(
      "  speakerQueue:  prod=%-6llu  cons=%-6llu  drop=%-4llu  hwm=%zu\n",
