...
if(NOT TARGET Kokoro::Kokoro)
    ...
    foreach(config IN ITEMS DEBUG RELEASE)
        ...
        if(EXISTS "${base_dir}")
            ...
            if(CMAKE_SYSTEM_NAME STREQUAL "Windows")
                set(onnx_lib_file "${onnx_dir}/lib/${ONNX_LIB_NAME}.lib")
                set(onnx_dll_file "${onnx_dir}/lib/${ONNX_LIB_NAME}.dll")
            elseif(CMAKE_SYSTEM_NAME STREQUAL "iOS")
                set(onnx_lib_file "${onnx_dir}/lib/onnxruntime.xcframework")
            elseif(CMAKE_SYSTEM_NAME STREQUAL "Darwin")
                set(onnx_lib_file "${onnx_dir}/lib/lib${ONNX_LIB_NAME}.dylib")
            else()
                set(onnx_lib_file "${onnx_dir}/lib/lib${ONNX_LIB_NAME}.so")
                if(NOT EXISTS "${onnx_lib_file}")
                    set(onnx_lib_file "${onnx_dir}/lib/lib${ONNX_LIB_NAME}.a")
                endif()
            endif()
