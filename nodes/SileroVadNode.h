...
struct SileroVadConfig {
  ...
  static void applyNoiseLevel(NoiseLevel level, float &outStart,
                              float &outStop) {
    switch (level) {
...
inline void readSafeString(std::ifstream &fin, std::string &str,
                           std::size_t length) {
...
template <typename T> inline void readSafeVal(std::ifstream &fin, T &val) {
...
inline ggml_tensor *buildEncoderLayer(ggml_context *ctx0, const VadModel &model,
                                      ggml_tensor *cur) {
...
inline ggml_tensor *buildLstmLayer(ggml_context *ctx0, VadContext &vctx,
  ...
  ggml_tensor *c_out = ggml_add(ctx0, ggml_mul(ctx0, f_t, vctx.c_state),
...
inline VadContext *loadModel(const std::string &modelPath) {
  ...
  model.version = std::to_string(major) + "." + std::to_string(minor) + "." +
                  std::to_string(patch);
  ...
  model.stft_forward_basis =
      createTensor(VAD_TENSOR_STFT_BASIS,
  ...
  for (int i = 0; i < 4; ++i) {
    auto *w = createTensor(
        ...
        ggml_new_tensor_3d(ctx, GGML_TYPE_F16, hparams.kernel_sizes[i],
                           hparams.encoder_in_channels[i],
                           hparams.encoder_out_channels[i]));
    auto *b =
        createTensor(static_cast<VadTensor>(VAD_TENSOR_ENC_0_BIAS + 2 * i),
                     ggml_new_tensor_1d(ctx, GGML_TYPE_F32,
                                        hparams.encoder_out_channels[i]));

    switch (i) {
    case 0:
      model.encoder_0_weight = w;
      model.encoder_0_bias = b;
      break;
    case 1:
      model.encoder_1_weight = w;
      model.encoder_1_bias = b;
      break;
    case 2:
      model.encoder_2_weight = w;
      model.encoder_2_bias = b;
      break;
    case 3:
      model.encoder_3_weight = w;
      model.encoder_3_bias = b;
      break;
    }
  }
  ...
  model.lstm_ih_weight =
      createTensor(VAD_TENSOR_LSTM_WEIGHT_IH,
                   ggml_new_tensor_2d(ctx, GGML_TYPE_F32,
                                      hparams.lstm_hidden_size, hstate_dim));
  model.lstm_ih_bias =
      createTensor(VAD_TENSOR_LSTM_BIAS_IH,
                   ggml_new_tensor_1d(ctx, GGML_TYPE_F32, hstate_dim));
  model.lstm_hh_weight =
      createTensor(VAD_TENSOR_LSTM_WEIGHT_HH,
                   ggml_new_tensor_2d(ctx, GGML_TYPE_F32,
                                      hparams.lstm_hidden_size, hstate_dim));
  model.lstm_hh_bias =
      createTensor(VAD_TENSOR_LSTM_BIAS_HH,
                   ggml_new_tensor_1d(ctx, GGML_TYPE_F32, hstate_dim));

  // Final conv
  model.final_conv_weight = createTensor(
      VAD_TENSOR_FINAL_CONV_WEIGHT,
      ggml_new_tensor_2d(ctx, GGML_TYPE_F16, hparams.final_conv_in, 1));
  model.final_conv_bias = createTensor(
      VAD_TENSOR_FINAL_CONV_BIAS, ggml_new_tensor_1d(ctx, GGML_TYPE_F32, 1));
  ...
  ggml_backend_buffer_t buf =
      ggml_backend_alloc_ctx_tensors(ctx, vctx->backend);
  ...
  ggml_init_params stateParams = {ggml_tensor_overhead() * 4, nullptr, true};
  ggml_context *ctx_state = ggml_init(stateParams);
  model.ctxs.push_back(ctx_state);

  vctx->h_state =
      ggml_new_tensor_1d(ctx_state, GGML_TYPE_F32, hparams.lstm_hidden_size);
  vctx->c_state =
      ggml_new_tensor_1d(ctx_state, GGML_TYPE_F32, hparams.lstm_hidden_size);
  ...
  vctx->allocr =
      ggml_gallocr_new(ggml_backend_get_default_buffer_type(vctx->backend));
  ...
  std::printf("[SileroVadNode] Loaded model: %s v%s  window=%d  context=%d\n",
              model.type.c_str(), model.version.c_str(), vctx->n_window,
              vctx->n_context);
  std::fflush(stdout);
...
inline float runInference(VadContext &vctx, const float *samples, int count) {
  ...
  if (count < vctx.n_window) {
    std::vector<float> zeros(static_cast<std::size_t>(vctx.n_window - count),
                             0.0f);
...
inline void freeContext(VadContext *vctx) {
  if (!vctx)
    return;
  for (auto *ctx : vctx->model.ctxs)
    ggml_free(ctx);
  for (auto buf : vctx->model.buffers)
  ...
  if (vctx->allocr)
    ggml_gallocr_free(vctx->allocr);
  if (vctx->buffer)
    ggml_backend_buffer_free(vctx->buffer);
  if (vctx->backend)
    ggml_backend_free(vctx->backend);
...
class SileroVadNode final : public ActiveNodeBase, public IVadNode {
public:
  explicit SileroVadNode(SileroVadConfig config)
      : cfg_(std::move(config)), pool_(cfg_.poolSize) {}

  ~SileroVadNode() override {
#if VOICE_RUNTIME_HAS_GGML
    if (vctx_)
      detail_silero::freeContext(vctx_);
#endif
  }
  ...
  bool initialize() override {
    ...
    if (cfg_.format.sampleRate != 16000 || cfg_.format.channels != 1 ||
        cfg_.format.frameMs != 10 ||
        cfg_.format.sampleFormat != SampleFormat::Int16) {
      std::fprintf(stderr, "[SileroVadNode] Unsupported format. "
    ...
    framesToAccumulate_ =
        (kSileroWindow + samplesPerFrame_ - 1) / samplesPerFrame_;
    // For 160-sample frames: ceil(512/160) = 4 → 640 total, take 512
    accumBuf_.resize(
        static_cast<std::size_t>(framesToAccumulate_ * samplesPerFrame_), 0.0f);
  ...
  void setNoiseLevel(SileroVadConfig::NoiseLevel level) {
    SileroVadConfig::applyNoiseLevel(level, cfg_.thresholdStart,
                                     cfg_.thresholdStop);
...
#if VOICE_RUNTIME_HAS_GGML
    if (vctx_)
      detail_silero::resetState(*vctx_);
#endif
    accumCount_ = 0;
  }
...
private:
  ...
  float runVadOnAccumulated() {
    ...
    for (const auto &pf : pendingFrames_) {
      const std::size_t n =
          static_cast<std::size_t>(pf->format.totalSamplesPerFrame());
      for (std::size_t i = 0; i < n && offset < accumBuf_.size(); ++i) {
        accumBuf_[offset++] = static_cast<float>(pf->pcm16[i]) / 32768.0f;
      }
    }

    if (useEnergyFallback_) {
      return energyVadProbability(accumBuf_.data(), static_cast<int>(offset));
    }
  ...
  void emitVadEvent(VadEvent::Type type, uint64_t timestampNs,
                    float confidence) {
    const char *label =
        (type == VadEvent::Type::SpeechStart) ? "SpeechStart" : "SpeechEnd";
